    ...
    def retrieve_and_concat(
        ...
        if memory_decoder_vec is not None:
            current_memories = self.memory_decoder.generate_memories(
                ...
                memory_decoder_vec[:, -2:], torch.Tensor([2]).to(memory_decoder_vec)
            )
        if self.generated_memories == current_memories:
            self.generated_memories
        else:
            self.generated_memories[0].extend(current_memories[0])
    ...
    def perform_search(
        ...
        if self.search:
            ...
            if len(search_queries) == 0:
                return [], torch.Tensor([[0]]).to(query_vec)
