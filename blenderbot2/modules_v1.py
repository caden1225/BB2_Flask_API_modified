...
from blenderbot2.sub_modules import (
    QueryGenerator,
    MemoryDecoder,
    clean_vec_with_dict,
    RetrievalType,
...
class BlenderBot2RagModel(RagModel):
    ...
    def __init__(self, opt: Opt, dictionary: DictionaryAgent, retriever_shared=None):
        from blenderbot2.blenderbot2 import RAG_MODELS
    ...
    def retrieve_and_concat(
        ...
        # current_memories = [[]]
        # if memory_decoder_vec is not None:
        #     current_memories = self.memory_decoder.generate_memories(
        #         # memory_decoder_vec, num_memory_decoder_vecs
        #         memory_decoder_vec[:, -2:], torch.Tensor([1]).to(memory_decoder_vec)
        #     )
        # if self.generated_memories[0] is None:
        #     self.generated_memories = current_memories
        # else:
        #     self.generated_memories.extend(current_memories)
        if self.should_generate_query:
            assert self.has_query_generator()
            retrieval_type, search_queries = self.query_generator.classify_retrieval(
                query_generator_vec, num_memories, self.generated_memories, skip_search
            )
    ...
    def perform_search(
        ...
        assert self.retriever is not None
        if self.search:
            # 20220523 修正know_access_method为full时,出现空search_query的问题
            if len(search_queries) == 0:
                return [], torch.Tensor([[0]]).to(query_vec.device)
...
class LongTermMemory(RagRetriever):
    ...
    def __init__(
        ...
        self.max_memories = opt.get('max_memories', 20)
