...
from config.myConfig import model_config_path
...
def multi_interactive(opt):
    ...
    SHARED['agent_ori'] = agent
    SHARED['world_loger'] = world_logger
    SHARED['world'] = create_task(opt, [human_agent, agent])


@register_script('interactive_api', aliases=['multi_api'], hidden=True)
class MultiInteractive(ParlaiScript):
    @classmethod
    def setup_args(cls):
        return setup_interactive_args()

    def run(self):
        return multi_interactive(self.opt)
